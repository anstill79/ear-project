...
/*   --------->  Backup prior to overhaul
function grabCustomNorms(choice) {
  //gets custom norms from page or from storage. Sets customs to the local storage
  localStorage.clear();
  return;
  let message;
  let five = "";
  ...
  let two = "";
  let four = "";
  const loadDisplay = document.getElementById("display norms");
  const retrieveLocal = localStorage.getItem("localNorms");

  //checks for empty local object and dumps in defaults if so
  if (!retrieveLocal) {
    localStorage.setItem("localNorms", JSON.stringify(norms))
  }
  ...
  if (choice === "set") {
    ...
    five = document.getElementById("norm500");
    one = document.getElementById("norm1000");
    two = document.getElementById("norm2000");
    four = document.getElementById("norm4000");
    message = "Norms updated from input boxes on screen. ";
...
console.log(localNorms.A);
return;

    //should only overwrite data when given an input, defaults or from the screen
    if (!localNorms.A) {
      localNorms.A = localStorage.getItem("localNorms.A");
    } else {
      localStorage.setItem("localNorms.A", localNorms.A);
    }
    if (!localNorms.B) {
      localNorms.B = localStorage.getItem("localNorms.B");
    } else {
      localStorage.setItem("localNorms.B", norms.B);
    }
    if (!localNorms.C) {
      localNorms.C = localStorage.getItem("localNorms.C");
    } else {
      localStorage.setItem("localNorms.C", norms.C);
    }
    if (!localNorms.D) {
      localNorms.D = localStorage.getItem("localNorms.D");
    } else {
      localStorage.setItem("localNorms.D", norms.D);
    }

console.log(`inside set fx ${localNorms.A}`);
    ...
    const localNorms = JSON.parse(retrieveLocal);
    five = parseInt(localNorms.A);
    one = parseInt(localNorms.B);
    two = parseInt(localNorms.C);
    four = parseInt(localNorms.D);
    message = "Norms updated from local storage. ";
    const timeStamp = new Date().toLocaleString();
    localStorage.setItem("localNorms.timeStamp", timeStamp);
    pushCustomNorms(five, one, two, four, message);
console.log(`inside get fx ${retrieveLocal}`);
  }
}
...
function pushCustomNorms(five, one, two, four, message) {
  ...
  if (alertFlag < 1) {} else {
    window.alert(`${message} ${alertFlag} of your custom norms ${alertFlag > 1 ? "were" : "was"} empty. Default norms applied to the empty frequencies.`);
  }

  fillNormTable(norms.A, norms.B, norms.C, norms.D);
  
  console.log(`inside push fx ${norms.A}`);
...
function fillNormTable(A, B, C, D) {
  ...
  const four = document.getElementById("normData4kcell");
  const target = document.getElementById("update-norm-time");
  const time = localStorage.getItem("localNorms.timeStamp");
